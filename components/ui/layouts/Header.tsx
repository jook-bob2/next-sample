...
export default function Header() {
	...
	const { state: langState, setLanguage } = useLanguage()
	...
	const { onLogoutSuccess, state: userState } = useUser()
				...
				<div className="container gnb-container">
					...
					<nav className={'gnb-nav-container ' + (menuOpen ? 'active' : '')}>
						...
						>
							...
							<li>
								<Link href={{ pathname: '/notice/notice-list' }}>
									<a>{'게시판'}</a>
								</Link>
								<ul className={`lnb ${menuOpen ? 'active' : ''}`}>
									<li>
							...
							*/}
							...
							<li>
								<a onTouchStart={() => handleClickSpreadMobMnu(1)}>
									{'게시판'} {spreadMobMnuList.includes(1) ? <MenuMinusIcon /> : <MenuPlusIcon />}
								</a>
